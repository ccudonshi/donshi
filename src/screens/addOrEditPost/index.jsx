...
import { Text, View, StyleSheet, Modal, Image, Alert } from 'react-native'
...
export function AddPostScreen({ isNeed, type }) {
    ...
    useEffect(() => { setModalVisible(false) }, [topic])
    useEffect(() => {
        getCurrentUserASync()
    ...
    const onSubmit = async () => {
        ...
        manager.addNewPost(images, newPost)
            .then(() => Actions.replace("home", { refresh: {} }))
    }
    return (
        PostView(onSubmit, user, setModalVisible, topic, modalVisible, setTopic, title, setTitle, text, setText, startDate, endDate, setStartDate, setEndDate, region, setRegion, images, setImages, type)
    )
...
export function EditPostScreen({ post }) { // Post
    ...
    const onSubmit = async () => {
        ...
        const manager = await AppHelper();
        manager.updatePost(images, newPost)
            .then(() => Actions.replace("home", { refresh: {} }))
    }
    return (
        PostView(
